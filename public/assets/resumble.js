var $ = window.$;
var res_script = $("#redirect");
var routepost = res_script.attr("data-route");
...
if ($fileUpload.length > 0 && $fileUploadDrop.length > 0) {
    ...
    if (!resumable.support) {
        ...
        resumable.on('fileAdded', function (file) {
            ...
            $('#sort').append('<li id="dv-' + file.uniqueIdentifier + '" class="resu mx-3 preview-file resumable-file-' + file.uniqueIdentifier + '" style="list-style: none;">' +
                ...
                '<div class="progress-bar progress-bar-striped resumable-file-' + file.uniqueIdentifier + ' resumable-file-progress" role="progressbar" style="width: 0%; background-color: #28A79E !important;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100">' +
                '</div>' +
                '</div>' +
                '<p class="text-center mt-0 text-center file-name position-absolute bottom-0">' +
                '<span class="resumable-file-name"></span>' +
                '</p>' +
                '</label>').parent('div').css('background-image', 'none').css('min-height', '10vw');
        ...
        resumable.on('fileSuccess', function (file, message) {
            ...
            var a = $('#sort .resu').length;
            $('.resumable-file-' + file.uniqueIdentifier).find('.upload-text').addClass('d-none');
            $('.upload-text').addClass('d-none');
            $('.resumable-file-' + file.uniqueIdentifier + ' .resumable-file-progress').html('Completed').remove('rounded-end').addClass('bg-success rounded-1');
        ...
        resumable.on('fileError', function (file, message) {
            ...
            $('.resumable-file-' + file.uniqueIdentifier + ' .resumable-file-progress').html('file could not be uploaded');
        });

        resumable.on('fileProgress', function (file) {
            $('.resumable-file-' + file.uniqueIdentifier + ' .resumable-file-progress').html(Math.floor(file.progress() * 100) + '%').css({
